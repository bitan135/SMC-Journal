...
export default function FeaturesPage() {
  return (
    <div className="min-h-screen bg-white text-[#0f172a] selection:bg-indigo-100">
      {/* Small Navbar */}
      <nav className="p-6 md:p-10 flex items-center justify-between border-b border-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-20 text-center">
        <span className="text-indigo-600 text-xs font-black uppercase tracking-[0.3em]">THE TOOLKIT</span>
        <h1 className="text-4xl md:text-7xl font-black mt-6 mb-8 tracking-tight text-[#0f172a]">Everything built for <br /> <span className="text-indigo-600">SMC Traders.</span></h1>
        <p className="text-xl font-bold text-slate-500 max-w-2xl mx-auto mb-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
          {features.map((f, i) => (
            <div key={i} className="p-10 rounded-[48px] bg-white border border-slate-100 hover:border-indigo-100 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all group overflow-hidden relative">
              ...
              <h3 className="text-2xl font-black mb-4 text-[#0f172a]">{f.title}</h3>
              <p className="text-slate-500 font-bold mb-8 leading-relaxed">{f.desc}</p>
        ...
        <section className="mt-40 p-12 md:p-24 rounded-[64px] bg-slate-50 border border-slate-100 text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-indigo-500/5 to-purple-600/5" />
          <h2 className="text-3xl md:text-5xl font-black mb-8 relative z-10 text-[#0f172a]">Stop guessing. Start journaling.</h2>
          <p className="text-lg font-bold text-slate-500 mb-12 max-w-2xl mx-auto relative z-10">
      ...
      <footer className="py-20 px-6 border-t border-slate-100 mt-20 text-center">
        <Link href="/" className="flex items-center justify-center gap-2 mb-8 opacity-40 hover:opacity-100 transition-opacity">
           <TrendingUp size={24} className="text-indigo-600" />
           <span className="text-lg font-black tracking-tighter text-[#0f172a]">SMC Journal</span>
        </Link>
